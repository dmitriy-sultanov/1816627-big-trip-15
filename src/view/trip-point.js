...
const createTripPointTemplate = (waypoint) => {
  ...
  <div class="event">
    <time class="event__date" datetime="${waypoint.dateFrom}">${startEventDay(waypoint.dateFrom)}</time>
    ...
    <div class="event__schedule">
      <p class="event__time">
        <time class="event__start-time" datetime="${waypoint.dateFrom}">${timeStart(waypoint.dateFrom)}</time>
        &mdash;
        <time class="event__end-time" datetime="${waypoint.dateTo}">${timeEnd(waypoint.dateTo)}</time>
      </p>
      <p class="event__duration">${waypoint.durationTime}</p>
    </div>
